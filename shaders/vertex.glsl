...
vec3 getOffset(vec3 p) {
	...
	vec3 temppos = rotation3dY(time * (0.5 + 0.5 * twist_scale) + length(pos.xy)) * p;
	// vec3 offset = fbm_vec3(temppos, 0.1, 0.0);
...
void main() {
	...
	vec3 offset = fbm_vec3(world_pos + offset0, 0.1, time * 0.1);
	...
	view_pos.xyz += position * size * (0.01 + 0.2 * particle_size);
